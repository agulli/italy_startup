...
graph TD
    ...
    B -->|Option 1: Heuristics| C[backend/scraper.py]
    B -->|Option 2: Gemini LLM| D[backend/scraper_llm.py]
    C -->|Output CSV| E[backend/italy_startups.csv]
    D -->|Output CSV| F[backend/italy_startups_llm.csv]
    F -->|Load & Geocode| G[backend/init_db.py]
    G -->|Geocoding Request| H[Nominatim API]
    H -->|Lat/Lng Coordinates| G
    G -->|Save to SQLite| I[backend/startups.db]
...
### 1. Scrapers
* **Heuristic Scraper ([scraper.py](file:///Users/antonio/code/GIT/italy%20startups/backend/scraper.py))**:
...
* **LLM Scraper ([scraper_llm.py](file:///Users/antonio/code/GIT/italy%20startups/backend/scraper_llm.py))**:
...
### 2. Database & Geocoding ([init_db.py](file:///Users/antonio/code/GIT/italy%20startups/backend/init_db.py))
